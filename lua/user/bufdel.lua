-- implementations of bdelete/bwipeout that don't screw up tab/window positioning
...
-- prompt user for choice, this captures the first character inputted after the prompt is shown and returns it
...
local function buf_kill(target_buffers, switchable_buffers, force, wipeout)
    ...
    if not force then
        for bufnr, _ in pairs(buf_is_deleted) do
            ...
            if vim.bo[bufnr].modified then
                local choice = char_prompt(string.format('No write since last change for buffer %d (%s).', bufnr, bufname(bufnr)), {'&Save', '&Ignore', '&Cancel'})
            ...
            elseif vim.bo[bufnr].buftype == 'terminal'
            and vim.fn.jobwait({vim.bo[bufnr].channel}, 0)[1] == -1 then
                local choice = char_prompt(string.format('Terminal buffer %d (%s) is still running.', bufnr, bufname(bufnr)), {'&Ignore', '&Cancel'})
    ...
    local windows = vim.tbl_filter(function(win) return buf_is_deleted[vim.api.nvim_win_get_buf(win)] ~= nil end, vim.api.nvim_list_wins())
    ...
    elseif vim.g.bufdelete_buf_filter ~= nil then
        ...
        switchable_buffers = vim.tbl_filter(function(buf) return vim.api.nvim_buf_is_valid(buf) and vim.bo[buf].buflisted end, vim.api.nvim_list_bufs())
    end

    -- filter buffers targeted for deletion from the buffer list
    local undeleted_buffers = vim.tbl_filter(function(buf) return not buf_is_deleted[buf] end, switchable_buffers)
    ...
    for bufnr, _ in pairs(buf_is_deleted) do
        if vim.api.nvim_buf_is_loaded(bufnr) then
            local use_force = force or vim.bo[bufnr].modified or vim.bo[bufnr].buftype == 'terminal'
            vim.api.nvim_exec_autocmds('User', { pattern = 'BDeletePre ' .. tostring(bufnr) })
            if wipeout then
                vim.cmd.bwipeout({ count = bufnr, bang = use_force })
            else
                vim.cmd.bdelete({ count = bufnr, bang = use_force })
            end
            vim.api.nvim_exec_autocmds('User', { pattern = 'BDeletePost ' .. tostring(bufnr) })
        end
    end
end
...
local function find_buffer_with_pattern(pat)
    for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
        if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_buf_get_name(bufnr):match(pat) then
            return bufnr
        end
    end
end
...
for k, v in pairs({ Bwipeout = true, Bdelete = false }) do
    vim.api.nvim_create_user_command(k, function(opts) M._buf_kill_cmd(opts, v) end, {
